...
//	console.log('sending off async request for link ' + link);
	// if (link in cache){
	// 	console.log('found something in cache! =');
	// 	swap_link_for_image($link, cache[link]);
	// }


// 	$.get(link, function(data){
// 		// if ($link.attr('href') === '/wiki/Canidae'){
// 		// 	debugger;
// 		// }
...
// //		var $data = $(data);
// 		var $images = $data.find('#bodyContent img')
// 						   .filter(is_valid_image);

// 		if ($images.length > 0){
// 			swap_link_for_image($link, $images.first());
// 		}

// 	});

//	debugger;
	link = 'http://en.wikipedia.org/w/api.php?format=json&action=query&generator=images&titles=' + token;
...
//function convert_to_pictures(start_ind, end_ind){
...
//			console.log('in queue not calling again');
			return;
		}
		called.push(elt);
